...
* **Scale-to-Fit (Auto-Fit)**: Automatically updates zoom and centering offsets on load or window resize to ensure the entire Crystal Sphere shell perfectly fills the available viewport. Handled via the standalone `AutoFitCalculator.ts` utility. The math bounds are calculated strictly against the `shellRadius` (ignoring distant Constellations that exceed it), and dynamically evaluates `Math.min(dimensions.width / 2, dimensions.height / 2 - 25)` to allow the sphere to physically touch the edges of the screen while preserving exactly 50px of vertical breathing room to accommodate the header Title text without clipping.
* **Constellations**: Draws constellation shapes natively on the chart. Custom SVGs are parsed and uniformly scaled (based on their **Size Class**), using `arcDegrees` as an additional uniform scale multiplier instead of arbitrarily stretching them. They automatically align tangentially to the orbit path at their initial angle. The visual representation includes a translucent black background, an intersecting white wireframe, and blinking, pseudo-randomized stars. Shape skeletons are pre-computed once during creation/import in the Custom Shape Wizard (using either Morphological Thinning for classic lines or Geometric Triangulation for volumetric meshes) and saved as a JSON sidecar. At render-time, the chart simply draws these pre-calculated static coordinates, filling detected triangles with a translucent glow to provide volumetric depth. **Wireframe Detail Level** selects the closest pre-computed Level of Detail (LOD) from 1 to 20, while **Star Count** independently determines how many stars are scattered across the nodes and edges of that graph. The elemental affinity dictates the star glow color. Astrolabe ships with a large set of pre-compiled constellation shapes, including classic D&D monster silhouettes and the 12 standard 5E character classes (Barbarian, Bard, Cleric, etc).
...
* **Renderer Styles (Stateless Classes)**: All actual rendering logic is pushed into swappable renderer style classes (e.g., `SimpleVertBookmarkRenderer`, `NavigationChartRenderer`). These implement the `INavigationChartRenderer` interface (or a similar common interface) exposing a `render(context)` method. Map aesthetic styles (like Vellum or Space) are configured via configuration objects (e.g. `NavigationChartStyleConfig`) rather than duplicated logic. The View Area instantiates the desired renderer and passes it a context object containing the canvas context, viewport state, and the celestial objects to draw. This structure allows seamless swapping of map aesthetic styles without re-implementing user interaction handling.
